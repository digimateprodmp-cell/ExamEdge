...
import type { NoteVolumeInput } from '@/services/content.service';
...
const EMPTY: FormValues = { titleEn: '', titleHi: '', isFree: true, price: '0' };

export function NoteVolumeFormDialog({
  title,
  initial,
  trigger,
  onSubmit,
  ...
  onSubmit: (input: NoteVolumeInput) => Promise<void>;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>{trigger ?? <Button size="sm"><Plus className="h-4 w-4" /> New note volume</Button>}</DialogTrigger>
      <DialogContent>
