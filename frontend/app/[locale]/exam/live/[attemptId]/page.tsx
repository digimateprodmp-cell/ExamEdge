...
import { useLiveTestAttempt } from '@/hooks/use-live-test-attempt';

export default function LiveExamPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  return (
    <div className="flex min-h-screen select-none flex-col">
      <Dialog open={!!pendingWarning} onOpenChange={(open) => !open && acknowledgeWarning()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-destructive">Integrity Warning</DialogTitle>
        ...
        <div className="flex-1 rounded-2xl border border-border bg-card p-5 sm:p-8">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-5">
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => toggleMarkForReview(question.testQuestionId)}>
                {t('markForReview')}
              </Button>
            </div>
            <div className="flex gap-2">
